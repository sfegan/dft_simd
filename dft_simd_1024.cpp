...
TEST(TestDFT_1024, FFTW_Aligned_One)
{
...
TEST(TestDFT_1024, FFTW_MisAligned_One)
{
...
TEST(TestDFT_1024, FFTW_Aligned_Eight)
{
...
TEST(TestDFT_1024, FFTW_Aligned_TransposedEight)
{
  float* xt = fftwf_alloc_real(nvec*nsamp);
  float* xf = fftwf_alloc_real(nvec*2*(nsamp/2 + 1));
  std::mt19937 core(12345);
...
TEST(TestDFT_1024, FFTW_SuperAligned_Eight)
{
...
TEST(TestDFT_1024, CODELET_AVX)
{
  ...
  for(int iloop=0; iloop<nloop; iloop++) {
    m256::dft_codelet_r2cf_1024(xt, xt+1, xf, xf+1, 2, 2, 2, 1, 0, 0);
    if(iloop==0) {
      for(int ifreq=0;ifreq<2*(nsamp/2 + 1);ifreq++) {
        std::cout << xf[ifreq][0] << ' ';
      }
...
TEST(TestDFT_1024, CODELET_AVX_FixedStride)
{
  ...
  for(int iloop=0; iloop<nloop; iloop++) {
    m256_FixedStride::dft_codelet_r2cf_1024(xt, xt+1, xf, xf+1, 2, 2, 2, 1, 0, 0);
    if(iloop==0) {
      for(int ifreq=0;ifreq<2*(nsamp/2 + 1);ifreq++) {
        std::cout << xf[ifreq][0] << ' ';
      }
...
TEST(TestDFT_1024, CODELET_AVX_Unroll2)
{
  ...
  for(int iloop=0; iloop<nloop/2; iloop++) {
    m256_Unroll2::dft_codelet_r2cf_1024(xt, xt+1, xf, xf+1, 2, 2, 2, 1, 0, 0);
    if(iloop==0) {
      for(int ifreq=0;ifreq<2*(nsamp/2 + 1);ifreq++) {
...
TEST(TestDFT_1024, CODELET_AVX_Unroll2_FixedStride)
{
  ...
  for(int iloop=0; iloop<nloop/2; iloop++) {
    m256_Unroll2_FixedStride::dft_codelet_r2cf_1024(xt, xt+1, xf, xf+1, 0, 0, 0, 1, 0, 0);
    if(iloop==0) {
      for(int ifreq=0;ifreq<2*(nsamp/2 + 1);ifreq++) {
        std::cout << xf[ifreq].first[0] << ' ';
      }
